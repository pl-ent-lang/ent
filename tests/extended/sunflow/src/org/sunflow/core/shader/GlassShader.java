...
public class GlassShader implements Shader {
    ...
    public Color getRadiance(ShadingState state) {
        ...
        if (inside && absorbtionDistance > 0) {
            ...
            absorbtion = Color.mul(-state.getRay().getMax() / absorbtionDistance, absorbtionColor.copy1().opposite()).exp();
    ...
    public void scatterPhoton(ShadingState state, Color power) {
        ...
        if (rnd < avgR) {
            ...
            if (state.isBehind() && absorbtionDistance > 0) {
                // this ray is inside the object and leaving it
                // compute attenuation that occured along the ray
                power.mul(Color.mul(-state.getRay().getMax() / absorbtionDistance, absorbtionColor.copy1().opposite()).exp());
            }
            if (arg < 0) {
                // TIR
                float dn = 2 * cos;
